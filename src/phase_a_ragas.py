...
def group_by_distribution(test_set: list[dict]) -> dict[str, list[dict]]:
    ...
    """
    # HƯỚNG DẪN GỐC (đã implement bên dưới): Task 1 - Nhóm theo distribution
...
def run_ragas_50q(answers: list[dict]) -> list[RagasResult]:
    ...
    """
    # HƯỚNG DẪN GỐC (đã implement bên dưới): Task 2 - Chạy RAGAS 50q
    # try:
    #     from src.m4_eval import evaluate_ragas
    # except ImportError:
...
def bottom_10(results: list[RagasResult]) -> list[dict]:
    ...
    """
    # HƯỚNG DẪN GỐC (đã implement bên dưới): Task 3 - Bottom 10
...
def cluster_analysis(results: list[RagasResult]) -> dict:
    ...
    """
    # HƯỚNG DẪN GỐC (đã implement bên dưới): Task 4 - Cluster analysis
    # matrix = {

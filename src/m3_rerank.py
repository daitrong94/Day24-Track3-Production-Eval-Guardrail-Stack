...
class CrossEncoderReranker:
    ...
    def _load_model(self):
        if self._model is None:
            # HƯỚNG DẪN GỐC (đã implement bên dưới): Load cross-encoder model
    ...
    def rerank(self, query: str, documents: list[dict], top_k: int = RERANK_TOP_K) -> list[RerankResult]:
        ...
        # HƯỚNG DẪN GỐC (đã implement bên dưới): Implement reranking
...
class FlashrankReranker:
    ...
    def rerank(self, query: str, documents: list[dict], top_k: int = RERANK_TOP_K) -> list[RerankResult]:
        # HƯỚNG DẪN GỐC (optional, đã implement bên dưới): from flashrank import Ranker, RerankRequest
        ...
        by_id = {d_i: d for d_i, d in enumerate(documents)}
        return [
            RerankResult(text=r["text"], original_score=by_id[r["id"]].get("score", 0.0),
                         rerank_score=float(r["score"]), metadata=by_id[r["id"]].get("metadata", {}), rank=i)

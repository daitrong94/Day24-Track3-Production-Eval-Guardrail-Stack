...
def chunk_semantic(text: str, threshold: float = SEMANTIC_THRESHOLD,
    ...
    """
    # HƯỚNG DẪN GỐC (đã implement bên dưới): Implement semantic chunking
...
def chunk_hierarchical(text: str, parent_size: int = HIERARCHICAL_PARENT_SIZE,
    ...
    """
    # HƯỚNG DẪN GỐC (đã implement bên dưới): Implement hierarchical chunking
    ...
    for parent in parents:
        ...
        for word in words:
            if len(current) + len(word) + 1 > child_size and current:
                children.append(Chunk(text=current.strip(), metadata={**metadata, "chunk_type": "child"}, parent_id=pid))
                current = ""
            current += word + " "
        if current.strip():
            children.append(Chunk(text=current.strip(), metadata={**metadata, "chunk_type": "child"}, parent_id=pid))
...
def chunk_structure_aware(text: str, metadata: dict | None = None) -> list[Chunk]:
    ...
    """
    # HƯỚNG DẪN GỐC (đã implement bên dưới): Implement structure-aware chunking
    # 1. metadata = metadata or {}
    ...
    def _flush():
        ...
        if combined:
            chunks.append(Chunk(
                text=combined,
                metadata={**metadata, "chunk_index": len(chunks), "section": current_header.lstrip("# ").strip(), "strategy": "structure"},
            ))
